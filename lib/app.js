...
export class App {
  ...
  addEventListener(type, listener) {
    if (changedRe.test(type)) {
      ...
      const state = statePool.get(this)
      const names = type.replace(changedRe, "").split(",").map(name => name.trim())
      const detectors = names.map(name => {
        const initialValue = getNestedItem(name, state)
        return changed(name, initialValue)
      })
      const original = listener
      const wrapper = state => {
        const result = detectors.map(detector => detector(state))
        if (result.some(r => r)) original(state)
      }
