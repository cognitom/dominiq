...
export class App {
  ...
  dispatch(name) {
    ...
    const observable = new Observable(async observer => {
      subscribed = true
      try {
        if (!name) return observer.complete()
        ...
        if (!result) return observer.complete()
        const isAsyncIterable =
          typeof result[Symbol.asyncIterator] === "function"
        if (isAsyncIterable) {
          // We don't use `for await ()` syntax here
          // for the environment which is not support ES2018
          for (;;) {
    ...
    // subscribes myself when no one subscribe me in sync
    setTimeout(() => {
      if (!subscribed) observable.subscribe(() => {})
    }, 0)
    return observable
  }
  start() {
